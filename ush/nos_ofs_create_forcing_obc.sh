...
## added by Zheng on 07/17/2018
    echo $HOMEnos/fix/$NESTED_PARENT_OFS/nos.$NESTED_PARENT_OFS.${OFS}.obc.center.nc >> nos.${OFS}.obc.ctl
## added by Zheng on 07/17/2018
...
#  typeset -Z3 fhr
  ...
  CYCLE=00
  reg="reg1"
  if [  "${OFS,,}" == "creofs" -o "${OFS,,}" == "sfbofs" -o "${OFS,,}" == "wcofs" -o "${OFS,,}" == "wcofs4" ]; then
      reg="reg2"
  elif [ "${OFS,,}" == "ciofs" ]; then
      reg="reg3"
  fi
...
##  allow to search back for maximum 2 days
      ...
      cp -p nos.${OFS}.obc.avg-wl* $COMOUT/.
