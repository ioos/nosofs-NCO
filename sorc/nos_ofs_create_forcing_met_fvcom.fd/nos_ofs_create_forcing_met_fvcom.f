C---------------------------------------------------------------------------------------------
C
C Fortran Program Name: nos_ofs_create_forcing_met_fvcom.f
C
...
C
C Abstract:
C   This program is used to generated surface forcing file for FVCOM from
C   NCEP operational forecast products of grib2 files. The wind is rotated to earth coordinates 
C   
C
C
C Usage: ./nos_ofs_create_forcing_met_fvcom  < control.ctl
C
C Input Parameters:
...
C
C Compiling/Linking Syntax: make -f Make_nos_ofs_create_forcing_met_fvcom
C
...
cc allocatable variables  unstructured ocean models
      ...
      integer, allocatable :: weightnodes(:,:)  ! NODEm,3)
      real, allocatable :: weights(:,:)       ! NODEm,3)
      integer n3(3)
      real w3(3)
C  temporary array
C  For regrid
...
CC
      ...
      real Uwind_L,Vwind_L,pair_L,tair_L,qair_L,prate_L,spfh_L
      real dswrf_L,dlwrf_L,uswrf_L,ulwrf_L,lhtfl_L,shtfl_L,nflux_L
...
! AJ 12/16/2011, SELFE requires more than time step than the end_time       
       IF(trim(OCEAN_MODEL) .EQ. 'SELFE')IHHE=IHHE+1
       read(5,*)IGRD
       read(5,'(a)')BUFFER
     ...
     & base_date(4)
! AJ 12/16/2011, SELFE doesn't use HOUR  to compute julian time      
       IF (TRIM(OCEAN_MODEL) .EQ. "SELFE" )base_date(4)=0 
...
!	  endif    
!       enddo
!       DBASE_MET_NOW=trim(adjustL(BUFFER))
!       OUTPUTFILE='met_fvcom.nc'
!       OUTPUTFILE1='hflux_fvcom.nc'
...
!____________________Set global attributes string of the NetCDF
      globalstr(1)= 'NCEP '//trim(DBASE)//' operational products'
      globalstr(2)= trim(OFS)//' surface forcing netCDF file'
      globalstr(3)= 'FVCOM grid (unstructured) surface forcing'
 ...
 !!________________________________________________
      IF (IGRD .GT. 0)THEN
        WRITE(*,*)'Reading FVCOM grid file ...',trim(GRIDFILE)
...
!        ALLOCATE(angm(NODEm) )
	...
	ENDIF  
        allocate(pairm(NODEm))
        allocate(tairm(NODEm))
        allocate(tdairm(NODEm))
        allocate(qairm(NODEm))
        allocate(tcdcm(NODEm))
        allocate(evpm(NODEm))
        allocate(dswrfm(NODEm))
        allocate(dlwrfm(NODEm))
        allocate(uswrfm(NODEm))
        allocate(ulwrfm(NODEm))
        allocate(lhtflm(NODEm))
        allocate(shtflm(NODEm))
        allocate(pratem(NODEm))
        allocate(spfhm(NODEm))
        allocate(nfluxm(NODEm))
        allocate(netlwrfm(NODEm))
        allocate(netswrfm(NODEm))
        ALLOCATE(hm(NODEm) )
        ALLOCATE(nv(NELEm,3))
        do n=1,NODEm
        ...
        enddo
        write(*,*)'number of total nodes in model domain =',NODEm
        write(*,*)'number of total elements in model domain =',NELEm
        IF (IGRD .EQ. 1)THEN
            allocate(weightnodes(NODEm,3))
            allocate(weights(NODEm,3))
	ENDIF       
      ENDIF  
      ...
      IF (trim(DBASE) .EQ. 'NDFD')THEN
          ...
          IF(FEXIST)THEN
            ...
            DO I=1,IM
              ...
              lon(i,j)=x
              lat(i,j)=y
              write(99,'(2f10.5)')lon(i,j),lat(i,j)
            ENDDO
            ENDDO
            CLOSE(81)
            close(99)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      IF (IGRD .GE. 1)THEN
        ...
        allocate(YOUT(NODEm) )
        allocate(ZOUT(NODEm ) )
        allocate(dummy1(NODEm,1 ) )
        allocate(dummy2(NODEm,1 ) )
        DO I=1,NODEm
         XOUT(I)=lonm(I)
	 YOUT(I)=latm(I)
         dummy1(I,1)=lonm(I)
...
c  max perturbation = 0.05 * grid size
     ...
     *        NODEm,XOUT,YOUT,ZOUT,weightnodes,weights,0)
!              do i=1,NODEm
!              write(*,*)i,(weights(i,k),weightnodes(i,k),k=1,3)
!              enddo
!              stop
	  ...
	  IF (ALLOCATED(TMP2D)) DEALLOCATE(TMP2D)
          ...
          allocate(Iout(NODEm,1))
          allocate(Jout(NODEm,1))
          allocate(outm(NODEm,1))
     ...
     &        NODEm,1,dummy1,dummy2,outm,Iout,Jout,0)
      ENDIF       
      pair_L=0.
      Uwind_L=0.
      Vwind_L=0.
...
!        print *,'TCDC_L=',tcdc_L
     ...
     1    lhtfl_L .GT. 0.0 .AND. shtfl_L .GT. 0.0 )nflux_L = 1.
      WRITE(*,*)'WRITE VARIABLE FLAGS FOR '//trim(DBASE)
      WRITE(*,*)Uwind_L,Vwind_L,pair_L,tair_L,qair_L,prate_L
      WRITE(*,*)spfh_L,dswrf_L,dlwrf_L,uswrf_L,ulwrf_L,lhtfl_L
      Write(*,*)shtfl_L,nflux_L,netlwrf_L,netswrf_L
     ...
     1	 nos_ofs_create_forcing_met_fvcom.f'
	 STOP
      ENDIF	  
      IF (IGRD .LE. 0)THEN
        IF (trim(OCEAN_MODEL) .EQ. 'FVCOM')THEN
          WRITE(*,*)'IGRD has to be greater than 0 for current FVCOM'
	   WRITE(*,*)'Please use IGRD=1'
	   STOP
        ELSEIF(trim(OCEAN_MODEL) .EQ. 'SELFE')THEN
           OUTPUTFILE1='sflux_air.nc'
           call nos_ofs_write_netcdf_wind_SELFE(OUTPUTFILE1,acid,1,
     &         IGRD,ISUB,JSUB,base_date,lonsub,latsub,time_start,
     &         uwind_L,vwind_L,pair_L,tair_L,spfh_L,globalstr)
           OUTPUTFILE2='sflux_rad.nc'
           call nos_ofs_write_netcdf_flux_SELFE(OUTPUTFILE2,fcid,1,
     &          IGRD,ISUB,JSUB,base_date,lonsub,latsub,time_start,
     &          dlwrf_L,dswrf_L,globalstr)

           OUTPUTFILE3='sflux_prc.nc'
           call nos_ofs_write_netcdf_prate_SELFE(OUTPUTFILE3,pcid,1,
     &          IGRD,ISUB,JSUB,base_date,lonsub,latsub,time_start,
     &          prate_L,globalstr)
	ENDIF
      ELSEIF (IGRD .GT. 0)THEN
        IF(trim(OCEAN_MODEL) .EQ. 'FVCOM' )THEN
           WRITE(BUFFER,810)IYRS,'-',IMMS,'-',IDDS,'T',IHHS,':',0,
     1	        ':00.000000'  
           Times(1)=trim(adjustL(BUFFER))
...
!	   call nos_ofs_met_write_netcdf_FVCOM(OUTPUTFILE,ncid,1,
!     &          IGRD,NODEm,NELEm,base_date,nv,lonm,latm,lonc,latc,
!     &          time_start,Times(1),uwind_L,vwind_L,nflux_L,dswrf_L,
!     &          pair_L,globalstr)
!	   call nos_ofs_met_write_netcdf_hflux_FVCOM(OUTPUTFILE1,
!     &          ncfluxid,1,IGRD,NODEm,NELEm,base_date,nv,lonm,latm,
!     &          lonc,latc,time_start,Times(1),Tair_L,qair_L,pair_L,
!     &          dlwrf_L,netswrf_L,nflux_L,tcdc_l,tdair_l,globalstr)


        ELSEIF(trim(OCEAN_MODEL) .EQ. 'SELFE' )THEN
          WRITE(*,*)'Not developed yet for SELFE'
	  STOP 
        ENDIF
...
!          IF(IND .GT. 0)IND_HOUR_END=IND-1
          ...
          ENDIF
          print *,'IYR,IMM,IDD,ICYC,IHH=',IYR,IMM,IDD,ICYC,IHH
          yearb=IYR
          monthb=IMM
          dayb=IDD
          hourb=IHH+ICYC
	  ...
	  ENDDO
          DO J=1,JSUB
	  DO I=1,ISUB
            pair(i,j)=tmp1(IMIN+I-1,JMIN+J-1) ! unit of pair FVCOM and SELFE  is Pa, ROMS uses milibar
	  ENDDO
	  ENDDO    
	ENDIF
...
!           WRITE(*,*)'The first file PRMSL.'//trim(DBASE)//' does not exist'
 ...
 !         IF(IND .GT. 0)IND_HOUR_END=IND-1
          READ(BUFFER(IND_DATE:IND_DATE+9),'(I4,3I2)')IYR,IMM,IDD,ICYC
          print *,'IYR,IMM,IDD,ICYC,IHH=',IYR,IMM,IDD,ICYC,
...
!     1    BUFFER(IND_HOUR:IND_HOUR_END)
	  ...
	  IF(time0 .GE. time)THEN
	    ...
	    ENDDO
            DO J=1,JSUB
	    DO I=1,ISUB
             IF (TRIM(OCEAN_MODEL) .EQ. "SELFE" )THEN  ! air temperature in SELFE is in Kelvin 
               tair(i,j)=tmp1(IMIN+I-1,JMIN+J-1)
	     ELSE  
               if (tmp1(IMIN+I-1,JMIN+J-1) .ge. -999.0)then
 ...
 !         IF(IND .GT. 0)IND_HOUR_END=IND-1
          ...
          IF(time0 .GE. time)THEN
            ...
            DO I=1,ISUB
              ...
              if (tcdc(i,j) .gt. -999.0) tcdc(i,j)=tcdc(i,j)/100. !percentage to fraction
            ENDDO
            ENDDO
          ELSE
            BACKSPACE (95)
...
!	  IF(abs(time0-time) .LE. 0.01)THEN
	    ...
	    DO I=1,ISUB
               ...
               if (tmp1(IMIN+I-1,JMIN+J-1) .GT. -999.0)then
                 if(qair(i,j).lt. 2.0)qair(i,j)=2.0      
	  ...
	  ELSE
	    ...
	    ENDDO
          ...
          DO J=1,JSUB
          DO I=1,ISUB
            write(22,*)I,J,qair(i,j)
          enddo
          enddo
        ELSE
         write(*,*)'relative humility file does not exist !!'
         write(*,*)'calculate RH from DPT and air Temp' 
C  calculate relative humility from dew point temperature according to formulas from http://www.cactus2000.de
     ...
     &                        (a3+tair(i,j)*a4) ) ))
		 ...
		 if(tmp1(i,j).lt. 2.0)tmp1(i,j)=2.0      
...
!            IF(IND .GT. 0)IND_HOUR_END=IND-1
            ...
            ELSE
              READ(BUFFER(IND_HOUR:IND_HOUR_END),*)IHH
            ENDIF
	    WRITE(*,*)'time=',IYR,IMM,IDD,IHH+ICYC 
            yearb=IYR
            monthb=IMM
            dayb=IDD
            hourb=ihh+ICYC
...
2324    continue	
	...
	IF(FEXIST .and. (.NOT. FOPEN))OPEN(96,file=trim(FNAME))
	    ...
	    ELSE
	      READ(BUFFER(IND_HOUR:IND_HOUR_END),*)IHH
	    ENDIF   
            print *,'EVP time=',IYR,IMM,IDD,ICYC,IHH
            yearb=IYR
            monthb=IMM
            dayb=IDD
            hourb=ihh+ICYC
...
C since Longitude is negative in west (minus 360), so the parameter Lov =LoV - 360 = -95
...
C                 FFJD=LAT_XX_P - latsub(I,J)
	...
	ENDIF  
	   ...
	   IF( (TIME .GT. TIME_START+0.001) .AND. (N .EQ. 1) )THEN  ! extra-interpolating to cover simulation period
              IF (trim(OCEAN_MODEL) .EQ. 'FVCOM')THEN
                WRITE(*,*)'IGRD has to be greater than 0 
     1		for current FVCOM'
	        WRITE(*,*)'Please use IGRD=1'
	        STOP
              ELSEIF (trim(OCEAN_MODEL) .EQ. 'SELFE')THEN
               call nos_ofs_write_netcdf_wind_SELFE(OUTPUTFILE1,acid,2,
     &         IGRD,ISUB,JSUB,base_date,lonsub,latsub,time_start,
     &          uwind,vwind,pair,tair,spfh,globalstr)
               call nos_ofs_write_netcdf_flux_SELFE(OUTPUTFILE2,fcid,2,
     &          IGRD,ISUB,JSUB,base_date,lonsub,latsub,time_start,
     &          dlwrf,dswrf,globalstr)
             call nos_ofs_write_netcdf_prate_SELFE(OUTPUTFILE3,pcid,2,
     &          IGRD,ISUB,JSUB,base_date,lonsub,latsub,time_start,
     &          prate,globalstr)
     ...
     1		for current FVCOM'
           ...
           ELSEIF (trim(OCEAN_MODEL) .EQ. 'SELFE')THEN
               call nos_ofs_write_netcdf_wind_SELFE(OUTPUTFILE1,acid,2,
     &         IGRD,ISUB,JSUB,base_date,lonsub,latsub,time,
     &          uwind,vwind,pair,tair,spfh,globalstr)
               call nos_ofs_write_netcdf_flux_SELFE(OUTPUTFILE2,fcid,2,
     &          IGRD,ISUB,JSUB,base_date,lonsub,latsub,time,
     &          dlwrf,dswrf,globalstr)
...
!             call INTERP_REMESH(NNSUB,XINP,YINP,ZINP,
     ...
     *          NODEm,XOUT,YOUT,ZOUT,weightnodes,weights,0)
              ...
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,ZOUT,weightnodes,weights,1)
 !             do i=1,NODEm
 !             write(*,*)i,(weights(i,k),weightnodes(i,k),k=1,3),zout(i)
 !             enddo
 !             stop
              IF (trim(OCEAN_MODEL) .EQ. 'FVCOM')THEN
               DO I=1,NELEm
                 uwindm(I)=(ZOUT(nv(I,1))+ZOUT(nv(I,2))+ZOUT(nv(I,3)))/3.0
               ENDDO
              ELSEIF(trim(OCEAN_MODEL) .EQ. 'SELFE')THEN
               DO I=1,NODEm
                 uwindm(I)=ZOUT(I)
...
!             call INTERP_REMESH(NNSUB,XINP,YINP,ZINP,
     ...
     *        NODEm,XOUT,YOUT,ZOUT,weightnodes,weights,1)
              IF (trim(OCEAN_MODEL) .EQ. 'FVCOM')THEN
               DO I=1,NELEm
                 vwindm(I)=(ZOUT(nv(I,1))+ZOUT(nv(I,2))+ZOUT(nv(I,3)))/3.0
               ENDDO
              ELSEIF(trim(OCEAN_MODEL) .EQ. 'SELFE')THEN
               DO I=1,NODEm
                 vwindm(I)=ZOUT(I)
            ...
            IF (pair_L .gt. 0.0)then
              ...
              IF (NTMP0 .ne. NTMP)then
                NTMP0=NTMP
                call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *           NODEm,XOUT,YOUT,pairm,weightnodes,weights,0)
              ENDIF
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,pairm,weightnodes,weights,1)
            ENDIF
            IF (tair_L .gt. 0.0)then
              ...
              IF (NTMP0 .ne. NTMP)then
                NTMP0=NTMP
                call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *          NODEm,XOUT,YOUT,tairm,weightnodes,weights,0)
              ENDIF
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,tairm,weightnodes,weights,1)
            ENDIF
            IF (qair_L .gt. 0.0)then
              ...
              IF (NTMP0 .ne. NTMP)then
                NTMP0=NTMP
                call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *          NODEm,XOUT,YOUT,qairm,weightnodes,weights,0)
              ENDIF
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,qairm,weightnodes,weights,1)
            ENDIF
            IF (tcdc_L .gt. 0.0)then
              ...
              IF (NTMP0 .ne. NTMP)then
                NTMP0=NTMP
                call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *          NODEm,XOUT,YOUT,tcdcm,weightnodes,weights,0)
              ENDIF
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,tcdcm,weightnodes,weights,1)
            ENDIF
            ...
            IF (tdair_L .gt. 0.0)then
              ...
              IF (NTMP0 .ne. NTMP)then
                NTMP0=NTMP
                call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *          NODEm,XOUT,YOUT,tdairm,weightnodes,weights,0)
              ENDIF
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,tdairm,weightnodes,weights,1)
            ENDIF
            IF (dswrf_L .gt. 0.0)then
              ...
              IF (NTMP0 .ne. NTMP)then
                NTMP0=NTMP
                call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *          NODEm,XOUT,YOUT,dswrfm,weightnodes,weights,0)
              ENDIF
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,dswrfm,weightnodes,weights,1)
            ENDIF
            ...
            IF (dlwrf_L .gt. 0.0)then
              ...
              IF (NTMP0 .ne. NTMP)then
                NTMP0=NTMP
                call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *          NODEm,XOUT,YOUT,dlwrfm,weightnodes,weights,0)
              ENDIF
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,dlwrfm,weightnodes,weights,1)
            ENDIF
            IF (nflux_L .gt. 0.0)then
              ...
              IF (NTMP0 .ne. NTMP)then
                NTMP0=NTMP
                call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *          NODEm,XOUT,YOUT,nfluxm,weightnodes,weights,0)
              ENDIF
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,nfluxm,weightnodes,weights,1)
            ENDIF
            IF (netswrf_L .gt. 0.0)then
              ...
              IF (NTMP0 .ne. NTMP)then
                NTMP0=NTMP
                call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *          NODEm,XOUT,YOUT,netswrfm,weightnodes,weights,0)
              ENDIF
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,netswrfm,weightnodes,weights,1)
            ENDIF
            IF (spfh_L .gt. 0.0)then
              ...
              IF (NTMP0 .ne. NTMP)then
                NTMP0=NTMP
                call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *          NODEm,XOUT,YOUT,spfhm,weightnodes,weights,0)
              ENDIF
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,spfhm,weightnodes,weights,1)
            ENDIF
            IF (prate_L .gt. 0.0)then
              ...
              IF (NTMP0 .ne. NTMP)then
                NTMP0=NTMP
                call INTERP_REMESH(NTMP,oned1,oned2,oned3,
     *          NODEm,XOUT,YOUT,pratem,weightnodes,weights,0)
              ENDIF
              call INTERP_REMESH(NTMP,oned1,oned2,oned3,
	...
	ELSEIF (IGRD .EQ. 4)THEN   !! spatial interpolation using nature neighbors
            ...
            IF (Uwind_L .gt. 0.0)then
              ...
              ENDDO
              ENDDO
              write(*,*)'NTMP of uwind= ',NTMP,N
              call INTERP_NNEIGHBORS(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,ZOUT)
              IF (trim(OCEAN_MODEL) .EQ. 'FVCOM')THEN
               DO I=1,NELEm
                 uwindm(I)=(ZOUT(nv(I,1))+ZOUT(nv(I,2))+ZOUT(nv(I,3)))/3.0
               ENDDO
     ...
     *        NODEm,XOUT,YOUT,ZOUT)
            ...
            IF (pair_L .gt. 0.0)then
              ...
              ENDDO
              ENDDO
              print *,'NTMP of pair= ',NTMP,N
              call INTERP_NNEIGHBORS(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,pairm)
            ENDIF
            IF (tair_L .gt. 0.0)then
              NTMP=0
              DO I=1,ISUB
              ...
              ENDDO
              ENDDO
              print *,'NTMP of TAIR= ',NTMP,N
              call INTERP_NNEIGHBORS(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,tairm)
            ENDIF
            IF (qair_L .gt. 0.0)then
              NTMP=0
              DO I=1,ISUB
              ...
              ENDDO
              call INTERP_NNEIGHBORS(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,qairm)
              print *,'NTMP of QAIR=',NTMP,N
            ENDIF
            IF (dswrf_L .gt. 0.0)then
              NTMP=0
              DO I=1,ISUB
     ...
     *        NODEm,XOUT,YOUT,spfhm)
            ...
            IF (tdair_L .gt. 0.0)then
              ...
              ENDDO
              ENDDO
              print *,'DPT NTMP=',NTMP,N
              call INTERP_NNEIGHBORS(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,tdairm)
            ENDIF
            IF (tcdc_L .gt. 0.0)then
              NTMP=0
              DO I=1,ISUB
              ...
              ENDDO
              ENDDO
              print *, 'NTMP of TCDC= ',NTMP,N
              call INTERP_NNEIGHBORS(NTMP,oned1,oned2,oned3,
     *        NODEm,XOUT,YOUT,tcdcm)
            ENDIF
            IF (prate_L .gt. 0.0)then
              NTMP=0
              DO I=1,ISUB
...
!     &        NODEm,1,dummy1,dummy2,outm,Iout,Jout,0)
     ...
     &        NODEm,1,dummy1,dummy2,outm,Iout,Jout,1)
            IF (trim(OCEAN_MODEL) .EQ. 'FVCOM')THEN
               DO I=1,NELEm
                 uwindm(I)=(outm(nv(I,1),1)+OUTM(nv(I,2),1)+OUTM(nv(I,3),1) )/3.0
               ENDDO
            ELSEIF(trim(OCEAN_MODEL) .EQ. 'SELFE')THEN
               DO I=1,NODEm
                 uwindm(I)=OUTM(I,1)
     ...
     &        NODEm,1,dummy1,dummy2,outm,Iout,Jout,1)
            IF (trim(OCEAN_MODEL) .EQ. 'FVCOM')THEN
               DO I=1,NELEm
                 vwindm(I)=(outm(nv(I,1),1)+OUTM(nv(I,2),1)+OUTM(nv(I,3),1) )/3.0
               ENDDO
            ELSEIF(trim(OCEAN_MODEL) .EQ. 'SELFE')THEN
               DO I=1,NODEm
                 vwindm(I)=OUTM(I,1)
     ...
     &        NODEm,1,dummy1,dummy2,outm,Iout,Jout,1)
          ...
          IF (tdair_L .gt. 0.0)then
            print *,'tdair'
            CALL INTERP_REGRID(iflag,ISUB,JSUB,lonsub,latsub,tdair,
     &        NODEm,1,dummy1,dummy2,outm,Iout,Jout,1)
            DO I=1,NODEm
               tdairm(I)=OUTM(I,1)
            ENDDO
          ...
          IF (dswrf_L .gt. 0.0)then
            print *,'dswrf'
            CALL INTERP_REGRID(iflag,ISUB,JSUB,lonsub,latsub,dswrf,
     &        NODEm,1,dummy1,dummy2,outm,Iout,Jout,1)
            DO I=1,NODEm
               dswrfm(I)=OUTM(I,1)
            ENDDO
...
!            DO I=1,ISUB
!            DO J=1,JSUB
!               write(66,*)lonsub(i,j),latsub(i,j),nflux(i,j)
!            ENDDO
!            ENDDO
            CALL INTERP_REGRID(iflag,ISUB,JSUB,lonsub,latsub,nflux,
     &        NODEm,1,dummy1,dummy2,outm,Iout,Jout,1)
            DO I=1,NODEm
               nfluxm(I)=OUTM(I,1)
            ENDDO
     ...
     &        NODEm,1,dummy1,dummy2,outm,Iout,Jout,1)
          ...
          IF (spfh_L .gt. 0.0)then
            print *,'spfh'
            CALL INTERP_REGRID(iflag,ISUB,JSUB,lonsub,latsub,spfh,
     &        NODEm,1,dummy1,dummy2,outm,Iout,Jout,1)
            DO I=1,NODEm
               spfhm(I)=OUTM(I,1)
            ENDDO
...
!     &          IGRD,NODEm,NELEm,base_date,nv,lonm,latm,lonc,latc,
!     &          Time,Times(1),uwindm,vwindm,nfluxm,dswrfm,pairm,
!     &          globalstr)
!	        call nos_ofs_met_write_netcdf_hflux_FVCOM(OUTPUTFILE1,
!     &          ncfluxid,2,IGRD,NODEm,NELEm,base_date,nv,lonm,latm,
!     &          lonc,latc,Time,Times(1),Tairm,qairm,pairm,dlwrfm,
!     &          netswrfm,nfluxm,tcdcm,tdairm,globalstr)
     ...
     &          netswrfm,dlwrfm,qairm,tdairm,tcdcm,evpm,pratem,globalstr)
		...
		WRITE(*,*)'Please use IGRD=0'
		STOP 
             ENDIF
           ENDIF
           IF (trim(OCEAN_MODEL) .EQ. 'FVCOM')THEN
               call GREGORIAN(jday0,yearb,monthb,dayb,hourb)
               IYR=INT(yearb)
...
!     &         netswrfm,nfluxm,tcdcm,tdairm,globalstr)
	...
	ENDIF         
300     CONTINUE
!        WRITE(23,'(25F12.4)')TIME,(dswrfm(i),uswrfm(i),dlwrfm(i),
!     1ulwrfm(i),lhtflm(I),shtflm(I),nfluxm(i),netswrfm(i),I=1,1,50)	  
      ENDDO
99    continue
     ...
     &          prate,globalstr)
        ...
        ELSEIF (IGRD .GT. 0)THEN
             IF (trim(OCEAN_MODEL) .EQ. 'FVCOM')THEN
	       jday=TIME_END
               WRITE(BUFFER,810)IYRE,'-',IMME,'-',IDDE,'T',IHHE,':',0,
     1	       ':00.000000'  
               Times(1)=trim(adjustL(BUFFER))
		print *,'time=',Times(1)
!	       call nos_ofs_met_write_netcdf_FVCOM(OUTPUTFILE,ncid,2,
...
!! close all NetCDF files for output
     ...
     1		for current FVCOM'
	        ...
	        STOP
           ELSEIF (trim(OCEAN_MODEL) .EQ. 'SELFE')THEN
             call nos_ofs_write_netcdf_wind_SELFE(OUTPUTFILE1,acid,3,
     &         IGRD,ISUB,JSUB,base_date,lonsub,latsub,time_end,
     &          uwind,vwind,pair,tair,spfh,globalstr)
             call nos_ofs_write_netcdf_flux_SELFE(OUTPUTFILE2,fcid,3,
     &          IGRD,ISUB,JSUB,base_date,lonsub,latsub,time_end,
     &          dlwrf,dswrf,globalstr)
             call nos_ofs_write_netcdf_prate_SELFE(OUTPUTFILE3,pcid,3,
     &          IGRD,ISUB,JSUB,base_date,lonsub,latsub,time_end,
     &          prate,globalstr)
           ENDIF
...
!     &          globalstr)
!	        call nos_ofs_met_write_netcdf_hflux_FVCOM(OUTPUTFILE1,
!     &          ncfluxid,3,IGRD,NODEm,NELEm,base_date,nv,lonm,latm,
!     &          lonc,latc,Time,Times(1),Tairm,qairm,pairm,dlwrfm,
!     &          netswrfm,nfluxm,tcdcm,tdairm,globalstr)
     ...
     &          netswrfm,dlwrfm,qairm,tdairm,tcdcm,evpm,pratem,globalstr)
      ...
      ENDIF
      WRITE(*,*)'nos_ofs_create_forcing_met COMPLETED SUCCESSFULLY'
      END
