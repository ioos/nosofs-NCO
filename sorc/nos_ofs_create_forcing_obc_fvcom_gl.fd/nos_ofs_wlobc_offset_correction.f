...
!      read(5,'(a100)')END_TIME
      ...
      IF(NNODE .ne. NOBC) THEN
        print *, 'NNODE and NOBC not consistent!! NNODE= ', NNODE,
     &  'NOBC= ', NOBC
        STOP
      ENDIF
     ...
     1	AGENCY_ID(N),lonSTA(N),latSTA(N),DATUM(N)
      ...
      IF(FEXIST) THEN
        ...
        DO N=1,NNODE
          ...
          print *,'old offset=',IDUMMY,WL_OFFSET_OLD(N)
        ENDDO 
        CLOSE(20)
      ELSE
        DO N=1,NNODE
           WL_OFFSET_OLD(N)=0.0
        ENDDO
        print*,CORRECTION_OLD,'not exist; default WL_OFFSET_OLD to 0'
      ENDIF
...
!	    IF (WL_OBS(I,N) .NE. -99999.9)THEN
     ...
     &	        (WL_OBS(I,N) .LE. BOUND_U)  )THEN
	    ...
	    ENDIF
	  ENDDO
        WRITE(6,*) '# of data point out of bound (+/-3sd):  ', NTR(I)-NTMP
  	  NTR(I)= NTMP
	  DO N=1,NTMP
             WL_OBS(I,N)=ONED1(N) 
	     RTIME(I,N)=ONED2(N)    
...
!	   GD=(WL_OBS(I,N)-WL_OBS(I,N-1))/(RTIME(I,N)-RTIME(I,N-1))/24.
	   ...
	   ENDIF
	  ENDDO
          WRITE(*,*) '# of data point rate of change > 0.7m/h:  ', NTR(I)-NTMP
	  DO N=1,NTMP
             WL_OBS(I,N)=ONED1(N) 
	     RTIME(I,N)=ONED2(N)    
	  ENDDO
...
!          print *, N, time_model(N), day_start, day_end
          if (time_model(N) .GE. day_start .and. time_model(N) .LE. day_end) THEN
          if (time_model(N) .GT. timelast)THEN
            ...
            write(10,'(F15.8,2F10.3)') time_model(N),(WL_MODEL(I,NREC),i=1,NSTA)
          ENDIF 
          ENDIF
        ENDDO	 
        write(*,*)'file=',trim(TMPFILE)
...
!     interpolate to the same time as observation
      IF (ALLOCATED(oned1)) DEALLOCATE(oned1)
      IF (ALLOCATED(oned2)) DEALLOCATE(oned2)
	...
	IF( (ABS(AVG_OBS(I)) .GT. 90.0) .OR. 
     1	    (ABS(AVG_MODEL(I)) .GT. 90.0) .OR. 
     1       NTR(I) .LE. NDAYS*240*0.8 ) THEN    !80% valid data
