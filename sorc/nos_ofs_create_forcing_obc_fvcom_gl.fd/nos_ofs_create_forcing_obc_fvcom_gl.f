...
!      GRIDFILE=TRIM(FIXofs)//'/'//TRIM(OFS)//'/'//trim(adjustL(BUFFER))
      ...
      print *,'WL_CORRECTION_CTL= ', trim(WL_CORRECTION_CTL)
      read(5,'(a200)')BUFFER
      do i=1,len_trim(BUFFER)
         if(BUFFER(i:I) .eq. "'" .or. BUFFER(i:I) .eq. '"')then
              BUFFER(i:I)=' '
      ...
      print *,'OBC_WL_OFFSET_FILE=', trim(CORRECTION_OLD)
      read(5,'(a200)')BUFFER
      do i=1,len_trim(BUFFER)
         if(BUFFER(i:I) .eq. "'" .or. BUFFER(i:I) .eq. '"')then
              BUFFER(i:I)=' '
...
!             T_OBS(I,NTR(I))=sst
     ...
     &          IHH,IMN,EL,ATMP,SST,SALN,COND,SWL
            ENDIF !station match
       ENDDO ! I=1,NSTA
...
C  ----------------------------------------------------------------------------
      ...
      DO I=1,NSTA
       ...
       DO N=1,NTR(I)
         ...
         ENDIF
       ENDDO 
          print*, 'NTMP=', NTMP,(WL_OBS(I,K),K=1,5)
        ...
        IF( WL_FLAG(I) .EQ. 0 )THEN
          ...
          ELSE
               ...
               IF( (IBKP1 .GT. 0) .AND.(NTR(IBKP1) .GT. 20) )THEN
                   ...
                   ENDDO
               ELSE
                     WRITE(*,*) 'No sufficient primary and back-up WL data for station',I
                     WRITE(ICORMS,*) 'No sufficient primary and back-up WL data for station',I 
                     DO IZ=INT(day_start),INT(day_start-1),-1
                       jday=IZ+jbase_date
                       call CALENDARDATE(jday,yearb,monthb,dayb,hourb,IYR,IMM,IDD,IHH,IMN,ISEC)
                       BUFFER1='/nos.'//trim(OFS)//'.obc.avg-wl.'//TRIM(NOS_ID(I))//'.dat'
                       WRITE(BUFRFILE,'(I4.4,2I2.2)') IYR,IMM,IDD 
                       FIN=trim(COMOUT00)//trim(OFS)//'.'//trim(BUFRFILE)//trim(BUFFER1)                      
                       INQUIRE(FILE=trim(FIN),EXIST=FEXIST)
                       IF (FEXIST) EXIT
...
CCC WRITE OUT OBS AVERAGE AS BACK-UP FOR NEXT CYCLE
          FIN='nos.'//trim(OFS)//'.obc.avg-wl.'//TRIM(NOS_ID(I))//'.dat'
          CLOSE(10)
          OPEN(10,FILE=trim(FIN))
          WRITE(10,*) AVG
          CLOSE(10)
...
C Calculate  WL_OFFSET
     ...
     & WL_CORRECTION_CTL,CORRECTION_OLD,START_TIME,NOBC, WL_OFFSET)
!-----------------------------------------------------------------------
