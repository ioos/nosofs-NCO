...
c  modified by A.ZHANG  03/06/2009
C INPUT:
C        NDATA  :    Number of data points
C	 XINP   : x/longitude coordinate of data points 
C        YINP   : y/latitude coordinate of data points
C	 AINP   : data value at x y point
C	 NINP   : Number of interpolated locations
C	 XOUT   : x/longitude coordinate of interpolated points 
C        YOUT   : y/latitude coordinate of interpolated points
C OUTPUT:	
C	 AOUT   : interploated value at XOUT,YOUT
C 
c-----------------------------------------------------------------------------
c                              
      subroutine INTERP_NNEIGHBORS(NDATA,XINP,YINP,AINP,
     & NINP,XOUT,YOUT,AOUT)
      
      real*4 XINP(NDATA),YINP(NDATA),AINP(NDATA)
      real*4 XOUT(NINP),YOUT(NINP),AOUT(NINP)

      parameter		(np_max=100000,nd_max=2,nt_max=2*np_max)
      parameter		(nh_max=100000,nnpn_max =50,nmax=3*nt_max+np_max)
      parameter		(nv_max=100000)
c
...
!       write(6,'(i5,3e15.5)') i,points(1,i),points(2,i),fvals(i)
      enddo
c
...
c
      ...
      ymin=minval(YOUT)
      ymax=maxval(YOUT)
      points(1,NDATA+1)=xmin-1.e-2*(xmax-xmin)
      points(2,NDATA+1)=ymin-1.e-2*(ymax-ymin)
      points(1,NDATA+2)=xmin-1.e-2*(xmax-xmin)
      points(2,NDATA+2)=ymax+1.e-2*(ymax-ymin)
      points(1,NDATA+3)=xmax+1.e-2*(xmax-xmin)
      points(2,NDATA+3)=ymax+1.e-2*(ymax-ymin)
      points(1,NDATA+4)=xmax+1.e-2*(xmax-xmin)
      points(2,NDATA+4)=ymin-1.e-2*(ymax-ymin)
      fvals(NDATA+1)=favg
      fvals(NDATA+2)=favg
      fvals(NDATA+3)=favg
      fvals(NDATA+4)=favg
      NDATA1=NDATA+4
!      DO I=1,NDATA1
!        write(56,*)points(1,i),points(2,i),fvals(i)
!      ENDDO	
...
c					2-D setup
      call nn2d_setup
     &     (NDATA1,nt_max,nh_max,np_max,nnpn_max,nmax,
...
!!      if(consistent)write(*,*)' neighbour matrix consistent'
c
c                                       write out vertices and neighbours
c					(for debug purposes)
c
      k = 0
!      open(21,file='list',status='unknown')
!      do 11 i=1,nt
!         write(21,*)i,' :',(vertices(j,i),j=1,3),
!     &              ' n:',(neighbour(j,i),j=1,3)
! 11   continue
...
!	  if( ISNAN(f) )write(*,*)xd,yd,AOUT(I),abs(AOUT(I)),I
!	  write(66,*)xd,yd,AOUT(I),I
      enddo
      return
      end
...
c
	...
	common/nnswitches/nnwrite
 	...
 	if(nnwrite)write(*,*)' Result of nn2Di search:'
...
c
	...
	common/nnswitches/nnwrite
 	...
 	if(nnwrite)write(*,*)' Result of nn2Do search:'
        inside_union = .false.
...
c					on the convex hull
	do 5 list = 1,nh
	   ...
	   if(dist.lt.centres(3,tri))then
	      if(nnwrite)write(*,*)' circle test found ',tri
	      call plot_tc(tri,points,vertices,centres)
	      inside_union = .True.
	      loc = tri
...
c
	...
	common/nnswitches/nnwrite
 	...
 	if(nnwrite)write(*,*)' Result of nn2Dr search:'
...
c
	...
	if(swap_nneighbors(points(1,v1),points(1,v2),
...
c------------------------------------------------------------------------
c
	Function swap_nneighbors(p1,p2,p3,p)
