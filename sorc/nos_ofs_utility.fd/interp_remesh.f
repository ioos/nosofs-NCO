      subroutine INTERP_REMESH(NDATA,XDATA,YDATA,ZDATA,
     & NINP,XINP,YINP,ZINP,weightnodes,weights,mode)

C INPUT:
C        mode   : 0-compute weights only; > 0 - compute interpolation value 
C        NDATA  :    Number of data points
C	 XDATA   : x/longitude coordinate of input data points 
C        YDATA   : y/latitude coordinate of input data points
C	 ZDATA   : input data value at XDATA YDATA point
C	 NINP   : Number locations to interpolate
C	 XINP   : x/longitude coordinate of interpolated points 
C        YINP   : y/latitude coordinate of interpolated points
C OUTPUT:	
C	 ZINP   : interploated value at XINP,YINP
C        weightnodes : node index for each interpolated point
C        weights :  the weights at the three nodes for each interpolated point    
	
C      implicit none
C      integer NDATA,NINP,MODE,I,K
      real*4 XDATA(Ndata), YDATA(Ndata),ZDATA(NDATA)
      real*4 XINP(NINP), YINP(NINP),ZINP(NINP)
C ---------------------------------------------------------------      
C  A. ZHANG  2009/03/20 
C add 4 more artifitial points to cover entire interpolated field and to 
C to avoid first three nodes lie on a line to cause colinear problem in remesh routine
C This method is also used in nature neighbors.
C Add random noise still cannot resolve colinear problem for remesh routine
C 
C Note that weights W1,W2, W3 of point P with respect to the triangle with vertices P1
C P2, and P3: W1 +W2+W3=1 and P=W1*P1+W2*P2+W3*P3
C W1 is a linear function of P which satisfies W1 = 1 at P = P1 and W1 = 0 if P is on
...
!        ydatamin=minval(YDATA)
!        ydatamax=maxval(YDATA)
        xmin=minval(XINP)
        xmax=maxval(XINP)
        ymin=minval(YINP)
        ymax=maxval(YINP)
!	ICOUNT=0
!	IF( (xmin .lt. xdatamin) .or. (ymin .lt. ydatamin) )THEN
!	   ICOUNT=ICOUNT+1
!           XTMP(ICOUNT)=xmin-1.e-2*(xmax-xmin)
!           YTMP(ICOUNT)=ymin-1.e-2*(ymax-ymin)
!	ELSEIF( (xmin .lt. xdatamin) .or. (ymax .gt. ydatamax) )THEN
!	   ICOUNT=ICOUNT+1
!           XTMP(ICOUNT)=xmin-1.e-2*(xmax-xmin)
!           YTMP(ICOUNT)=ymax+1.e-2*(ymax-ymin)
!	ELSEIF( (xmax .gt. xdatamax) .or. (ymax .gt. ydatamax) )THEN
!	   ICOUNT=ICOUNT+1
!           XTMP(ICOUNT)=xmax+1.e-2*(xmax-xmin)
!           YTMP(ICOUNT)=ymax+1.e-2*(ymax-ymin)
!	ELSEIF( (xmax .gt. xdatamax) .or. (ymin .lt. ydatamin) )THEN
!	   ICOUNT=ICOUNT+1
!           XTMP(ICOUNT)=xmax+1.e-2*(xmax-xmin)
!           YTMP(ICOUNT)=ymin-1.e-2*(ymax-ymin)
!        ENDIF
        XTMP(1)=xmin-1.e-2*(xmax-xmin)
        YTMP(1)=ymin-1.e-2*(ymax-ymin)
        XTMP(2)=xmin-1.e-2*(xmax-xmin)
        YTMP(2)=ymax+1.e-2*(ymax-ymin)
        XTMP(3)=xmax+1.e-2*(xmax-xmin)
        YTMP(3)=ymax+1.e-2*(ymax-ymin)
        XTMP(4)=xmax+1.e-2*(xmax-xmin)
        YTMP(4)=ymin-1.e-2*(ymax-ymin)
	DO I=1,NDATA
	  XTMP(I+4)=XDATA(I)
	  YTMP(I+4)=YDATA(I)
	ENDDO 
!	NNEW= NDATA+ICOUNT
        CALL REMESH(NDATA+4,XTMP,YTMP,NINP,XINP,YINP,
     &  weightnodes,weights)
!        CALL REMESH(NDATA+4,XDATA,YDATA,NINP,XINP,YINP,
!     &  weightnodes,weights)
	...
	AVG=AVG/NDATA
        ...
        DO I=1,NINP
          n3= weightnodes(I,:)
          w3= weights(I,1:3)
	  SUMW=ABS(w3(1))+ABS(w3(2))+ABS(w3(3))
          IF( (w3(1) .LT. 0.0) .OR. (w3(2) .LT. 0.0) .OR.
     &	      (w3(3) .LT. 0.0) .OR. SUMW .GT.3)THEN
             ZINP(I)=-99999.9
	     GOTO 10
	  ENDIF      
          ZINP(I)=w3(1)*ZTMP(n3(1))+w3(2)*ZTMP(n3(2))+w3(3)*ZTMP(n3(3))
10        continue
        ENDDO
...
C Provides the weights to interpolate the data found on the lond,latd grid to 
c model grid.  The output of this routine is the remeshed data
c  in the array datam
c
c This first triangulates lond, latd.
c Then it calculates the wieghts at each point in the model grid lon,lat
c which will give a linear interpolation of the three surrounding nodes of
c data in the triangulated lond,latd arrays.
c The weights are then applied.
c 
c Return the weightnodes, weights arrays for other usage.
c (Might want to leave out the step which uses weights for another routine.)
c
c To do the final conversion of your data
c  Put into the mainline something like
c      do i=1,NNmodel
c       n3= weightnodes(i,:)
c       w3= weights(i,1:3)
c       bathy(i)=sum(w3*bathyd(n3))
c      enddo 
c

       subroutine REMESH(NNdata,lond,latd,NN,lon,lat,
     & weightnodes,weights)
     
       implicit none
       integer NNdata,NN
      real*4 lond(NNdata), latd(NNdata)

C Model variables.  These are the target of the datad > data
C ie the result of Remeshing datad
      real*4 lon(NN),lat(NN)

      integer weightnodes(NN,3),trieta(9,2*NNdata)
      real weights(NN,3)
      
C scratch data for the remesh processing
c     integer, allocatable ::  trieta(:,:)
c      integer, allocatable :: weightnodes(:,:)
c      real*4, allocatable :: weights(:,:) 
      integer ntri, n3(3)
      real*4 w(3),x3(3),y3(3),w3(3)   
      integer  i,j,k,NT     

C*C  Create weights for coversion of data locations to model locations
c  Use lon,lat(NN) and lond,latd(NNdata)  
C to construct weights to interpolate datad onto the model locations.

C   Create a connectivity mesh for lond,latd  
C  trieta(nele,3)=   (n1,n2,n3)  counter clockwise nodes around element
C

c greg insert
!        do i=1,NNdata
!        write(85,*) 'lond= ', lond(i), 'latd= ',latd(i), 'i= ',i
!        end do
!        close(89)
!        do i=1,NN
!        write(99,*) 'lon= ', lon(i), 'lat= ',lat(i), 'i= ',i
!        end do
!        close(99)

c     allocate(trieta(9,2*NNdata))
      write(*,*) " start triangulate "
      call triangulate(NNdata,lond,latd,NT,trieta)
c returns the triangle info trieta(9,NT) =
c  n1 n2 n3   tri1 tri2 tri3   arc1 arc2 arc3  
c  nodes of triangle    adjacent triangles   other arc stuff
       
c      allocate(weightnodes(NN,3))
c      allocate(weights(NN,3))    
      
      ntri = 7
      do i = 1,NN
      call findsurroundtri(ntri,lon(i),lat(i),lond,latd,trieta)
!      write(*,*) "after findsurround", i,ntri,lon(i),lat(i)
      n3=trieta(1:3,ntri)
      x3=lond(n3)
      y3=latd(n3)
c      write(*,*) i,ntri,lon(i),lat(i),lond(n3),latd(n3)      
      call calcweights(lon(i),lat(i),x3,y3,w)    
      weightnodes(i,:)=n3
      weights(i,:)=w
!      write(*,*)'weights at node : ',I,w(1),w(2),w(3)
      enddo
      write(*,*) "after findsurround", NN,ntri,lon(NN),lat(NN)

c a debugging output of the element array for the data
c      if ( 2 .eq. 2) then
c       write(22,*) "tri=ones(",NT,",9);"
c       write(22,*) "tri(:)=[ "
c      do i=1,NT
c       write(22,'(9i6)') (trieta(j,i),j=1,9)
c      enddo
c       write(22,*) " ];"
c      endif
...
c gregs
         if ( 2 .eq. 2) then
c            write(*,*) "tri=ones(",NT,",9);"
c            write(*,*) "tri(:)=[ "
         do i=1,NT
c            write(*,'(9i6)') (trieta(j,i),j=1,9)
         enddo
c            write(*,*) " ];"
         endif
...
c       calcweights(x,y,x3,y3,w)    
c  given the three points x3,y3(1:3) surrounding the point x,y
c  calculate the linear interpolation weights  w(1:3)
c      % From A simple guide to finite elements   elements.m
!      subroutine calcweights(x,y,x3,y3,w)

      subroutine calcweights(x4,y4,x5,y5,w)
          
      real*4 x4,y4,x5(3),y5(3),w(3)   !! added by AJ ZHANG 03/02/2009
      real*8 x,y,x3(3),y3(3)
      real*8 Ad,b(3),c(3),d(3),a,Fx,Fy
 !! added by AJ ZHANG   03/02/2009     
      x=x4
      y=y4
      do i = 1,3
         x3(i)=x5(i)
 ...
 !!  end of AJ ZHANG      
      ...
      Ad = .5*( x3(1)*y3(2)-x3(1)*y3(3) 
     &    + x3(2)*y3(3)-x3(2)*y3(1)
     &    + x3(3)*y3(1)-x3(3)*y3(2)  )
      b = ( / x3(2)*y3(3)-x3(3)*y3(2), x3(3)*y3(1)-x3(1)*y3(3),
     &       x3(1)*y3(2)-x3(2)*y3(1) / )
      c = ( / y3(2)-y3(3), y3(3)-y3(1), y3(1)-y3(2) / )
      d = ( / x3(3)-x3(2),  x3(1)-x3(3), x3(2)-x3(1) / )
...
c      subroutine triangulate(N,X,Y, NT,LTRI)
C  Given random data in arrays X(1:N), Y(1:N) 
C  triangulate a convex region using the X,Y data
c  Output is   LTRI(1:9,NT)
c  NT = number of triangles
c  LTRI(1:9,NT) = n1,n2,n3  tri1,tri2,tri3, arc1,arc2,arc3
c  n1,n2,n3  indices of nodes in X,Y
c tri1,tri2,tri3  indices of triangles which are adjacent to this triangle
c  tri1 shares the  X(n1),X(n2) side with the present triangle.
      subroutine triangulate(N,X,Y, NT,LTRI)
      ...
      INTEGER IER, i
C
C  NMAX needs to be bigger than any arrays you might pass to this
C  routine
      PARAMETER (NMAX=200000, NCMAX=5, NTMX=2*NMAX, N6=6*NMAX,
     &           LWK=2*NMAX, NROW=9)
C
C Array storage:
C
     ...
     &        LPTR(N6),  NODES(LWK)
      ...
      if (N.gt. NMAX) then
        write(*,*) ' triangulate is not dimensioned big enough, n=',N
	write(*,*) ' crash out...'
	stop
      endif
	
      write(*,*) 'Inside triangulate N=',N
      NCC=1
      LCC(1)=9
      
C input is N, X(n),Y(n)
...
c LNEW = list length plus 1  
c    NODES, NDOES(n+1),DS are grabage

c  called to construct  LTRI
c  NT = number of triangles
c  LTRI(1:9,NT) = n1,n2,n3  tri1,tri2,tri3, arc1,arc2,arc3
!! ZAJ 03/26/2009
!! TRLIST will stop if N <= 12 while NCC>=1, in this case, no constraint curves for triangulation 
      IF(N .LT. 12)NCC=0 
...
c    The adjacent triangles need to be changed a bit
c so that the line segment from n1-n2 corresponds to tri1 etc..
      do i = 1,NT
       m= LTRI(6,i)
       LTRI(6,i)=LTRI(5,i)
       LTRI(5,i)=LTRI(4,i)
      ...
      write(*,*) "out of triangulate"

      return
      end


C find the surrounding triangle of x,y in the mesh xn,yn,tri
C  tri is (9,*) from triangulate
C tri(1:3,*)  node1,2,3 for a triangle
C tri(4:6,*)  triangles which share side with center tri
C   4 corresponds to side  2-1,  5 to 3-2  6 to 1-3  
C  righthandedness is essential 
C
C Walk along the mesh from triangle to triangle
C Always to toward side where Cross product of the side 
C with the search location is  negative.
C When all cross products are positive we surround the search location.
C 
      subroutine findsurroundtri(i,x,y,xn,yn,tri)
      integer i,tri(9,*),ilast
      real x,y,xn(*),yn(*)
      
c      call findsurroundtri(ntri,lon(i),lat(i),loneta,lateta,trieta)
      
      real vx,vy,ax,ay,cross1, cross2,cross3
      integer keepgoing

c Want to find the triangle where the cross products of all three
c vectors is Positive.  
cV1 x A1    xn2-xn1,yn2-yn1  X x-xn1,y-yn1
c xn(tri(2,i))-xn(tri(1,i)), yn(tri(2,i))-yn(tri(1,i))  cross
c    x-xn(tri(1,i)),y-yn(tri(1,i))
      
      keepgoing = 1
      do while (keepgoing==1)
        ilast=i
        vx= xn(tri(2,i))-xn(tri(1,i)) 
        vy= yn(tri(2,i))-yn(tri(1,i))
        ax= x-xn(tri(1,i))
        ay= y-yn(tri(1,i))
c	write(*,*) i,tri(2,i),tri(1,i)
c	write(*,*) xn(tri(2,i)),xn(tri(1,i)),x
c	write(*,*) yn(tri(2,i)),yn(tri(1,i)),y
        Cross1 = vx*ay-vy*ax
        if (Cross1<0) then
           i=tri(1+3,i)
        else
           vx= xn(tri(3,i))-xn(tri(2,i))
	   vy= yn(tri(3,i))-yn(tri(2,i))
           ax= x-xn(tri(2,i))
	   ay= y-yn(tri(2,i))
           Cross2 = vx*ay-vy*ax
           if (Cross2<0) then
              ...
              vx= xn(tri(1,i))-xn(tri(3,i))
	      vy= yn(tri(1,i))-yn(tri(3,i))
              ax= x-xn(tri(3,i))
	      ay= y-yn(tri(3,i))
              Cross3 = vx*ay-vy*ax
              if (Cross3<0) then
...
c   All crosses are positive.  Found the surrounding triangle. Stop      
                keepgoing = 0

c      write(*,*) " found positives",Cross1,Cross2,Cross3

              endif
           endif
        endif
        if (i==0) then
c  Walked off the edge of the map.  Use the last triangle and stop.
	 i=ilast
         keepgoing = 0
        endif
...
C
    4 NB = K
      ...
      RETURN
      END
      SUBROUTINE CIRCUM_remesh (X1,Y1,X2,Y2,X3,Y3,RATIO, XC,YC,CR,
     .                   SA,AR)
      LOGICAL RATIO
...
C            triangle.
C
C Modules required by CIRCUM_remesh:  None
C
C Intrinsic functions called by CIRCUM_remesh:  ABS, SQRT
C
C***********************************************************
C
      INTEGER I
...
C
   12 NT = 0
      IER = 1
      WRITE(*,*)'nvalid input parameter'
      print *,'NT= ',NT,'IER= ',IER,'return 12'
      RETURN
C
...
C
   13 NT = 0
      IER = 2
      WRITE(*,*)'Invalid triangulation data structure: TRLIST'
      WRITE(*,*)'I1 is a neighbor of I2, but I2 not a neighbor of I1'
      RETURN
      END
...
C             are also returned.
C
C  CIRCUM_remesh - Computes the area, circumcenter, circumradius,
