...
!
!svn $Id: get_nudgcoef.F 857 2017-07-29 04:05:27Z arango $
!================================================== Hernan G. Arango ===
...
!
     ...
     &                    GRID(ng) % rmask,                             &
     &                    CLIMA(ng) % M2nudgcof)
        IF (FoundError(status, nf90_noerr, 210,                         &
     &                 "ROMS/Utility/get_nudgcoef.F")) THEN
          exit_flag=2
          ioerror=status
          RETURN
...
!
     ...
     &                       nVarDim = nvdim,                           &
     &                       nVarAtt = nvatt)
        IF (FoundError(exit_flag, NoError, 246,                         &
     &                 "ROMS/Utility/get_nudgcoef.F")) RETURN
        DO i=1,nvatt
          IF (TRIM(var_Aname(i)).eq.'units') THEN
            tunits=TRIM(var_Achar(i))
...
!
     ...
     &                    GRID(ng) % rmask,                             &
     &                    CLIMA(ng) % M3nudgcof)
        IF (FoundError(status, nf90_noerr, 268,                         &
     &                 "ROMS/Utility/get_nudgcoef.F")) THEN
          exit_flag=2
          ioerror=status
          RETURN
