...
16    READ(20,*)BUFFER 
     ...
     &  TS_S_1(N),TS_SID_2(N),TS_S_2(N)
        ...
        hOBC(N)=hm(IOBC(N) )
        if(lonOBC(N) .GT. 180.)lonOBC(N)=lonOBC(N)-360.
      ENDDO
17    READ(20,*)BUFFER 
      BUFFER=trim(adjustL(BUFFER))
      LL=len_trim(BUFFER)
	...
	ENDDO  
        if(loneOBC(N) .GT. 180.)loneOBC(N)=loneOBC(N)-360.
      ENDDO
      DO I=1,NEOBC
         I1=nv(JOBC(I),1)
         I2=nv(JOBC(I),2)
...
11      read(40,'(a200)',end=12)BUFFER
     ...
     1	       'vertical parameters KBm and KB are not consistent' 
	  ...
	  IF(IND .GT. 0)THEN
!            IF (TRIM(STYPE) .EQ. 'GENERALIZED' )THEN	 
!               WRITE(*,*)'record does not include DU info in '
!	       STOP
!	    ENDIF   
!	  ELSE
             IND=INDEX(BUFFER,'=')
...
!-----------------------------------------------------------------------
!  Define sigma coordinates in domain [-1 < sc < 0] at vertical points.
!-----------------------------------------------------------------------
!
      ...
      ELSEIF (TRIM(STYPE) .EQ. 'GENERALIZED' )THEN
        DO I=1,NEOBC
          ...
          ELSE
            siglev_ele(I,1)=0.0
            DR=(HeOBC(I)-DU-DL)/HOBC(I)/(KB-KU-KL-1)
            DO K=2,KU+1
...
C------------------------------------------------------------------
      NREC=NINT( (jdaye-jdays)*24/DELT)+1
      DO N=1,NREC
	 TIME_clim(N)=((jdays-jbase_date)*24.+(N-1)*DELT)/24.0
      ENDDO
      NT_clim=NREC
      DO N=1,NSTA
         IYR=IYRS
...
!	 FIN=TRIM(FIXofs)//'/'//TRIM(OBC_CLIM_FILE)
	 FIN=TRIM(OBC_CLIM_FILE)
	    ...
	    WRITE(*,*)TRIM(OBC_CLIM_FILE)
         WRITE(ICORMS,'(a)')'Climatologic T&S file (WOA) is not found'
         WRITE(ICORMS,'(a)')TRIM(OBC_CLIM_FILE)
         WRITE(ICORMS,'(a)')'CRITICAL FAILURE IN CREATING OBC' 
 	    STOP
	 ENDIF  
         FILE_TS=TRIM(FIN)
...
!               WL(I,J,ICOUNT)=diff
               WL(I,J,ICOUNT)=diff + 0.2   !! nontidal WL is about 20 cm lower based on comparison made by Zizang
              ENDDO	
              ENDDO
...
c------------------------------------------------------------------
300   CONTINUE
      print *,'Begin horizontal interpolation for T & S'
      DO K=1,KMAX
         WRITE(*,*)'RTOFS VERTICAL LAYER K= ',K, 'DEPTH= ',depth(K)
         IF (IGRD .EQ. 1 .OR. IGRD .EQ. 4)THEN  
             NDUM=0
             DO NS=1,NSELECT_PARENT
     ...
     &        NEOBC,1,lon_in,lat_in,outm,Iout,Jout,0)
      ...
      DO K=1,KMAX
         IF (IGRD .EQ. 1 .OR. IGRD .EQ. 4)THEN
             ...
             ENDDO
             NDATA=NDUM
             print *,'U Data number at vertical layer ',K,' is ',NDATA
!            IF (IGRD .EQ. 1 .AND. NDATA .GT. 3)THEN
             IF (IGRD .EQ. 1 )THEN
                write(*,*)'COMPUTE WEIGHTS AND NODES FOR REMESH !!!'
                call INTERP_REMESH(NDATA,XINP,YINP,ZINP,
     *          NEOBC,loneOBC,lateOBC,ZOUT,weightnodes,weights,0)
             ENDIF
         ENDIF
         DO N=NSTR,NEND
           N0=N-NSTR+1
     ...
     &               NEOBC,loneOBC,lateOBC,ZOUT,weightnodes,weights,0)
                  ...
                  IF (IGRD .EQ. 1 )THEN   
                     call INTERP_REMESH(NDATA,XINP,YINP,ZINP,
     &               NEOBC,loneOBC,lateOBC,ZOUT,weightnodes,weights,1)
     ...
     &               NEOBC,loneOBC,lateOBC,ZOUT)
                ...
                ENDDO
                IF (NDUM .LE. 3)THEN
                  WRITE(*,*)'there is no parent data point atk=',K,depth(k)
                  write(*,*)'extropolate upper level (k-1) to K'
                  IF(K .GT. 1)THEN
                    DO I=1,NEOBC
...
!      hc=min(max(hmin,0.0),TCLINE)
         ...
         DO I=1,NEOBC
           ...
           DO K=1,KMAX
             IF (ABS(ueOBC(I,K,N)) .GE. 90.0 .AND. K .GT. 1)THEN
                oned3(K)=ueOBC(I,K-1,N)
             ELSE
                oned3(K)=ueOBC(I,K,N)
             ENDIF
             IF (ABS(veOBC(I,K,N)) .GE. 90.0 .AND. K .GT. 1)THEN
                oned4(K)=veOBC(I,K-1,N)
             ELSE
                oned4(K)=veOBC(I,K,N)
             ENDIF
           ENDDO
      ...
      OPEN(45,file='TS_NCOM_FVCOM.dat')
      DO I=1,NOBC
           DO K=1,KBM
              sigma(K)=0.5*(siglev(I,K)+siglev(I,K+1))
	      zsigma(K)=sigma(K)*HOBC(I)
              IF (zsigma(k) .LT. 0.0)zsigma(k)=-zsigma(k)
	      WRITE(45,*)'I K=',I,K,sigma(K),zsigma(k),HOBC(I)
           ENDDO
         DO N=1,NTMAX
            WRITE(45,'(I5,2x,10F9.3)')N,TIMEOBC(N)
...
!      IF (TRIM(DBASE_TS) .EQ. 'WOA05' )THEN  ! temp
      ...
      ENDDO
      NTMAX=NREC
CC  Computer U, V, ubar, and vbar at the center of elements for FVCOM
      IF (trim(OCEAN_MODEL) .EQ. "FVCOM" )THEN
        DO N=1,NTMAX
        do I=1,NEOBC
          avgu=0.0
...
!            if(abs(WL(I,J,ICOUNT)) .GE. 99.0)WL(I,J,ICOUNT)=-99999.9
...
C   Begin interpolating in time to DELT for UBAR and VBAR
      ...
      ENDIF     !  END OF IF (TRIM(DBASE_WL) .EQ. 'RTOFS')THEN
c------------------------------------------------------------------
...
CCC For SFBOFS Set nontidal water levels = 0, but specified by observations
...
C ----------------------------------------------------------------------------
      ...
      XMAJOR=0.0
!      DELT_PRD=DELT
      DELT_PRD=0.1  !  6 mintues time interval
...
C      write(15,*) 'ID        lat     lon   year  mon day hour
C     *  Disch   Tem SST   Salinity'
...
C  ----------------------------------------------------------------------------
		 ...
		 IF(SALN .EQ. -99999.99)THEN
		   IF( (SST .NE.-99999.99) .AND. (COND .NE. -99999.99) )THEN
		      SALN=SAL(COND,SST,0.0)
		   ENDIF   
		 ENDIF  
	         IF (SALN .EQ. -99999.99)GOTO 715
                 IF(    (NTR_S(I) .GT. 1)
...
C  ----------------------------------------------------------------------------
	         ...
	         IF (EL .EQ. -99999.99)GOTO 8720
                 IF(    (NTR(I) .GT. 1)
     &	             .and. (dayj .LE. RTIME(I,NTR(I)) ) )GOTO 8720 
	         NTR(I)=NTR(I)+1
                 RTIME(I,NTR(I) )=dayj
	         WL_OBS(I,NTR(I))=EL-DATUM(I)    !convert WL from USGS datum (e.g. NAVD88) to MSL for USGS stations
8720             CONTINUE         
C  ----------------------------------------------------------------------------
C  GET CONDUCTIVITY AND SALINITY
...
!-----------------------------------------------------------------------
     ...
     &   	        (TRIM(NOS_ID(I)) .EQ. '11455420')    )THEN
	              SWL_OBS(I,N)=AS(IBKP)*SWL_OBS(IBKP,N)  !!+0.22   !offset adjustment for Port Chicago
                    ELSE
	       ...
	       ENDIF
	     ENDIF
C For USGS gauges of 11337190 and 11455420, use climatological nontidal water levels AJ 03/01/2013	      
             IF( NTR(I) .LE. 20 )THEN  !!  2 can be changed depending upon how many data are required to be available
               NREC=NINT( (jdaye-jdays)*24/DELT)+1
               IF (ALLOCATED(oned1)) DEALLOCATE(oned1)
               IF (ALLOCATED(oned2)) DEALLOCATE(oned2)
...
620              CONTINUE
	         ...
	         IF(ONED3(ICOUNT) .LT. day_end )then
  	         ...
  	         ENDIF
	         CALL lineararray(NREC,TIME_M,oned1,ICOUNT,oned3,oned4)
	         NTR(I)=NREC
  	         DO N=1,NTR(I)
	            RTIME(I,N)=TIME_M(N)
                    SWL_OBS(I,N)=ONED4(N)
...
!-----------------------------------------------------------------------
     ...
     &	            (zeta_time(N1) .LE. TIME2) )THEN
	        ...
	        ENDIF
	     ENDDO
	  IF(N0 .LT. 5)THEN  ! for USGS data time interval=15 min, CO-OPS data time interval=6 min
	       WRITE(*,*)'real time water level data is insufficient, N0=',N0
...
905             CONTINUE
	     ...
	     AVGERR(I)=AVG
	     WRITE(*,*)'mean diff. of obs and model WL OBC=',I,AVGERR(I),N0
             DO N=1,N0
  	     ...
  	     AVG=0.0
             DO N=1,N0
	          ...
	          IF(ONED3(N) .EQ. TS_time(N1) )THEN
	            ONED1(N)=ONED4(N)-TEMPOBC_M(GRIDID_STA(I),1,N1)! avg=Tobs - NCOM at surface at the corresponding grid
                    AVG=AVG+ONED1(N)
		    GOTO 925
	          ENDIF	  
                ENDDO
	     ...
	     CALL lineararray(N0,ONED3,oned4,NTR_S(I),ONED1,ONED2)
  	     AVG=0.0
             DO N=1,N0
                DO N1=1,NTMAX
	          IF(ONED3(N) .EQ. TS_time(N1) )THEN
	            ONED1(N)=ONED4(N)-SALTOBC_M(GRIDID_STA(I),1,N1)! avg=Sobs - NCOM at the corresponding grid
                    AVG=AVG+ONED1(N)
		    GOTO 8925
	          ENDIF	  
                ENDDO
...
C do not correct RTOFS salt OBC due to no obs. available near OB     
                IF( (TRIM(OFS) .EQ. 'ngofs') .or. 
     &              (TRIM(OFS) .EQ. 'NGOFS' ) )THEN		
	          AVGERR_S(ID1)=0.0
     	          S_OBS(ID1,N)=0.0
                ENDIF
                IF( (TRIM(OFS) .EQ. 'sfbofs') .or.
...
!	  if(I .eq. NOBC)FOUT='node_last.prd'
	...
	write(*,*)'WL OBC is done'	   
        ...
        STATUS = NF_INQ_VARID(NCID,'UA_amp',IDVAR)
        STATUS = NF_GET_VAR_REAL(NCID,IDVAR,tide_amp)
	WRITE(*,*)'min & max UA_amp=',minval(tide_amp),maxval(tide_amp)
	...
	WRITE(*,*)'min & max UA_pha=',minval(tide_epoc),
     ...
     &        XMAJOR,AMP,EPOC,FOUT,ONED1,ONED2)
          DO N=1,NREC
	     ubarOBC(I,N)=ubarOBC(I,N)+ONED1(N)   ! non-tide + tidal prediction
	  ENDDO
        ENDDO
        STATUS = NF_INQ_VARID(NCID,'VA_amp',IDVAR)
        STATUS = NF_GET_VAR_REAL(NCID,IDVAR,tide_amp)
	WRITE(*,*)'min & max VA_amp=',minval(tide_amp),maxval(tide_amp)
	...
	WRITE(*,*)'min & max VA_pha=',minval(tide_epoc),
     ...
     &        XMAJOR,AMP,EPOC,FOUT,ONED1,ONED2)
          DO N=1,NREC
	     vbarOBC(I,N)=vbarOBC(I,N)+ONED1(N)   ! non-tide + tidal prediction
	  ENDDO
        ENDDO
	write(*,*)'UA OBC is done'	   
        DO I=1,4
          DIMS(I)=1
        ENDDO	
	...
	DO K=1,DIMS(2)
     ...
     &        XMAJOR,AMP,EPOC,FOUT,ONED1,ONED2)
          DO N=1,NREC
	     ueOBC_M(I,K,N)=ueOBC_M(I,K,N)+ONED1(N)   ! non-tide + tidal prediction
	  ENDDO
...
C 
	...
	DO K=1,DIMS(2)
     ...
     &        XMAJOR,AMP,EPOC,FOUT,ONED1,ONED2)
          DO N=1,NREC
	     veOBC_M(I,K,N)=veOBC_M(I,K,N)+ONED1(N)   ! non-tide + tidal prediction
	  ENDDO
...
810         FORMAT(I4.4,A1,I2.2,A1,I2.2,A1,I2.2,A1,I2.2,A10)
        ...
        ENDDO
C  final check if there is alnormal values or NaN in the OBC variables
C  check WLOBC
        DO I=1,NOBC
	DO N=1,NTMAX_WL
	    IF( (ABS(WLOBC(I,N)) .GE. 99.0) .OR. (IEEE_IS_NAN(WLOBC(I,N))) )THEN
               WRITE(6,*) 'FATAL ERROR: Abnormal values in WLOBC at I',I,'N= ',N 
               STOP
	    ENDIF
	  ENDDO
	ENDDO
...
C  check ubarOBC
        DO I=1,NEOBC
	DO N=1,NTMAX
	    IF( (ABS(ubarOBC(I,N)) .GE. 99.0) .OR. (IEEE_IS_NAN(ubarOBC(I,N))) )THEN
               WRITE(6,*) 'Abnormal values in ubarOBC at I',I,'N= ',N 
               ubarOBC(I,N)=0.0
	    ENDIF
	  ENDDO
	ENDDO
...
C  check vbarOBC
        DO I=1,NEOBC
	DO N=1,NTMAX
	    IF( (ABS(vbarOBC(I,N)) .GE. 99.0) .OR. (IEEE_IS_NAN(vbarOBC(I,N))) )THEN
               WRITE(6,*) 'FATAL ERROR: Abnormal values in vbarOBC at I',I,'N= ',N 
               vbarOBC(I,N)=0.0
	    ENDIF
	  ENDDO
	ENDDO
...
C Check TEMPOBC_M at surface K=1
        ...
        DO I=1,NOBC
	DO N=1,NTMAX
	    IF( (ABS(TEMPOBC_M(I,K,N)) .GE. 99.0) .OR. (IEEE_IS_NAN(TEMPOBC_M(I,K,N))) )THEN
               WRITE(6,*) 'FATAL ERROR: Abnormal values in TEMPOBC at I',I,'N= ',N 
               STOP
	    ENDIF
	  ENDDO
	ENDDO
	...
	DO N=1,NTMAX
	    IF( (ABS(TEMPOBC_M(I,K,N)) .GE. 99.0) .OR. (IEEE_IS_NAN(TEMPOBC_M(I,K,N))) )THEN
               WRITE(6,*) 'Abnormal values in TEMPOBC at I',I,'N= ',N,'K= ',K
               WRITE(*,*)'Replace using good value at the above layer'
...
C Check SALTOBC_M at surface K=1
        ...
        DO I=1,NOBC
	DO N=1,NTMAX
	    IF( (ABS(SALTOBC_M(I,K,N)) .GE. 99.0) .OR. (IEEE_IS_NAN(SALTOBC_M(I,K,N))) )THEN
               WRITE(6,*) 'FATAL ERROR: Abnormal values in SALTOBC at I',I,'N= ',N 
               STOP
	    ENDIF
	  ENDDO
	ENDDO
	...
	DO N=1,NTMAX
	    IF( (ABS(SALTOBC_M(I,K,N)) .GE. 99.0) .OR. (IEEE_IS_NAN(SALTOBC_M(I,K,N))) )THEN
               WRITE(6,*) 'Abnormal values in SALTOBC at I',I,'N= ',N,'K= ',K
               WRITE(*,*)'Replace using good value at the above layer'
...
C Check UeOBC_M at surface K=1
        ...
        DO I=1,NEOBC
	DO N=1,NTMAX
	    IF( (ABS(UEOBC_M(I,K,N)) .GE. 99.0) .OR. (IEEE_IS_NAN(UEOBC_M(I,K,N))) )THEN
               WRITE(6,*) 'FATAL ERROR: Abnormal values in UEOBC at I',I,'N= ',N 
               UEOBC_M(I,K,N)=0.0
	    ENDIF
	  ENDDO
	ENDDO
	...
	DO N=1,NTMAX
	    IF( (ABS(UEOBC_M(I,K,N)) .GE. 99.0) .OR. (IEEE_IS_NAN(UEOBC_M(I,K,N))) )THEN
               WRITE(6,*) 'Abnormal values in UEOBC at I',I,'N= ',N,'K= ',K
               WRITE(*,*)'Replace using good value at the above layer'
...
C Check VeOBC_M at surface K=1
        ...
        DO I=1,NEOBC
	DO N=1,NTMAX
	    IF( (ABS(VEOBC_M(I,K,N)) .GE. 99.0) .OR. (IEEE_IS_NAN(VEOBC_M(I,K,N))) )THEN
               WRITE(6,*) 'FATAL ERROR: Abnormal values in VEOBC at I',I,'N= ',N 
               VEOBC_M(I,K,N)=0.0
	    ENDIF
	  ENDDO
	ENDDO
	...
	DO N=1,NTMAX
	    IF( (ABS(VEOBC_M(I,K,N)) .GE. 99.0) .OR. (IEEE_IS_NAN(VEOBC_M(I,K,N))) )THEN
               WRITE(6,*) 'Abnormal values in VEOBC at I',I,'N= ',N,'K= ',K
               WRITE(*,*)'Replace using good value at the above layer'
...
! imode=3
	...
	CLOSE(67)
     ...
     &      (TRIM(OFS) .EQ. 'SFBOFS' ) )THEN		
          CLOSE(68)
   	  OPEN(68,file='obc_sfbofs_el.dat')
	  DO N=1,NREC
	    WRITE(68,3334)TIME_M(N),(WLOBC(I,N),I=1,NOBC,10),
     1	    WLOBC(97,N),WLOBC(103,N)
	  ENDDO
          CLOSE(68)
	  OPEN(68,file='obc_sfbofs_temp.dat')
	  DO N=1,NREC
	    WRITE(68,3334)TIME_M(N),(tempOBC_M(I,1,N),I=1,NOBC,10),
     1	    tempOBC_M(97,1,N),tempOBC_M(103,1,N)
	  ENDDO
          CLOSE(68)
	  OPEN(68,file='obc_sfbofs_salt.dat')
	  DO N=1,NREC
	    WRITE(68,3334)TIME_M(N),(saltOBC_M(I,1,N),I=1,NOBC,10),
     1	    saltOBC_M(97,1,N),saltOBC_M(103,1,N)
	  ENDDO
          CLOSE(68)
